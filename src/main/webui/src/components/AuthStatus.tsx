...
const AuthStatus: React.FC = () => {
    ...
    const handleLogout = () => {
        /**
         * FIX: Wir setzen den State hier NICHT manuell auf false.
         * Da window.location.href einen kompletten Seitenwechsel/Reload auslöst,
         * würde ein manuelles Setzen des States nur ein kurzes Flackern der UI
         * verursachen, bevor der Browser die Seite ohnehin verlässt.
         */
    ...
    return (
        <div className="auth-status flex items-center space-x-4">
          ...
          <span className="text-slate-300 text-sm">
            Angemeldet als: <span className="font-semibold text-white">{username}</span>
          </span>
                    <button
                        onClick={handleLogout}
                        className="p-1.5 bg-red-600 text-white text-sm rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-50 transition-colors duration-200 flex items-center justify-center"
                        title="Logout"
                    >
                        <LogOut size={18} />
                    </button>
