    ...
    useEffect(() => {
        fetch('/api/user')
            ...
            .then(data => {
                ...
                // Preloading des Auto-Moduls
                // Wenn der Nutzer angemeldet ist, lade das Modul schon mal im Hintergrund
                if (loggedIn) {
                    import('../pages/Auto').catch(console.error);
                }
            })
                        ...
                        {isMenuOpen && (
                            <div className="absolute top-full left-0 pt-2 w-48">
                                <div className="bg-slate-800/90 backdrop-blur-md border border-slate-700 rounded-md shadow-lg">
                                    ...
                                    <Link to="/strompreis" className="block px-4 py-2 text-sm text-slate-300 hover:bg-slate-700">Strompreis</Link>
                                    {isAuthenticated && (
                                        <Link to="/auto" className="block px-4 py-2 text-sm text-slate-300 hover:bg-slate-700">Auto</Link>
                                    )}
